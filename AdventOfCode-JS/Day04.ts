...
export var solve = input => {
    ...
    var result1 = phrases.reduce((count, phrase) => noMatches(phrase) ? count + 1 : count, 0);
    console.log(`Part 1: ${result1}`);
    var result2 = phrases.reduce((count, phrase) => noAnagrams(phrase) ? count + 1 : count, 0);
...
function noMatches(phrase) {
    ...
    return !words.some((word) => {
        if (checkedWords.some(w => w == word)) return true;
...
function noAnagrams(phrase) {
    const words = phrase.split(/\s+/);
    const checkedWords = [];
    const isAnagram = (word1, word2) => word1.length == word2.length &&
        word1.split('').sort().join('') == word2.split('').sort().join('');

    return !words.some((word) => {
        if (checkedWords.some(w => isAnagram(w, word))) return true;
        checkedWords.push(word);
        return false;
    });
}
