...
export var solve = input => {
    ...
    console.log(`Part 2: ${notBalanced ? notBalanced.balanceReport() : "Everything is balanced."}`);
};
...
class Tower {
    ...
    isBalanced() {
        ...
        var balance = true;
        this.getChildren().map((c) => { return c.totalWeight; }).forEach((w) => {
            if (!sharedWeight) {
                sharedWeight = w;
    ...
    balanceReport() {
        if (this.isBalanced()) return this.name + " is balanced.";
            ...
            const imbalancer = this.getImbalancer();
    ...
    getImbalancer() {
        ...
        let correctTotal = children.reduce((correct, c) => {
            if (seen.indexOf(c.totalWeight) >= 0) correct = c.totalWeight;
            else seen.push(c.totalWeight);
            return correct;
        }, 0);

        let imbalancer = children.find(c => c.totalWeight != correctTotal);
        imbalancer.correctWeight = imbalancer.weight - (imbalancer.totalWeight - correctTotal);
        return imbalancer;
    }

    getChildren() {
        return this.children.map(n => this.towers[n] || (this.towers[n] = new Tower(n, this.towers)));
    }
